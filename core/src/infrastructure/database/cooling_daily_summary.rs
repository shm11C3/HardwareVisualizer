...
/// falls in, for the ambient pairing join
/// (`cooling_thermal_delta_daily_summary`) and its cursor. The archive
/// tick stamps every table of one write cycle with a single instant, so
/// exact equality would now very nearly work - but bucketing to the
/// minute is what the join actually *means*, and rows archived before
/// that fix still carry sub-minute drift between the two sides.
pub(super) fn sqlite_minute_key(epoch_milliseconds: &str) -> String {
...
pub(super) fn raw_timestamp_bound(instant: &DateTime<Utc>, direction: i64) -> String {
...
async fn select_archive_minutes_for_range_from_pool(
  ...
  // The ambient side is deliberately *not* joined here. The Thermal Delta
  // is folded per ambient source from its own paired read
  // (`cooling_thermal_delta_daily_summary::select_thermal_delta_minutes_for_range`),
  // because one per-minute ambient value averaged across sources would
  // mix two sensor placements into a ΔT no sensor observed (#2062).
  let sql = format!(
    "SELECT
...
mod tests {
  ...
  use super::super::test_schema::{
    COOLING_DAILY_SUMMARY_DDL, DATA_ARCHIVE_DDL, DATA_ARCHIVE_TIMESTAMP_INDEX_DDL,
    create_tables,
  };

  /// The hardware archive with the timestamp index the shipped schema
  /// carries.
  async fn setup_data_archive(pool: &SqlitePool) {
    create_tables(pool, &[DATA_ARCHIVE_DDL, DATA_ARCHIVE_TIMESTAMP_INDEX_DDL]).await;
  }
  ...
  fn range_query_sql(start: &DateTime<Utc>, end: &DateTime<Utc>) -> String {
    ...
    format!(
      "SELECT DATA_ARCHIVE.timestamp, DATA_ARCHIVE.cpu_avg
       FROM DATA_ARCHIVE
  ...
  async fn without_the_index_the_range_query_would_scan_the_archive() {
    ...
    let pool = SqlitePool::connect("sqlite::memory:").await.unwrap();
    create_tables(&pool, &[DATA_ARCHIVE_DDL]).await;
