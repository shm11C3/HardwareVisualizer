...
/// The one place the native family deliberately does *not* reproduce the
/// SQLite family, measured here so the difference is a decision on the record
...
/// epoch key on either side. SQLite groups it under a NULL bucket, sqlx decodes
/// that NULL as 0, and bucket 0 is outside every range a user can ask for: the
/// row is inside the requested range and simply absent from the answer. The
/// series under-reports and says nothing.
///
/// The native query refuses instead and names the value. For the screen the
/// outcome is the same one row of missing data; for whoever has to fix it, one
/// is findable and the other is not.
async fn compare_an_unreadable_timestamp(database: &NativeDatabase) {
  ...
  assert!(
    ...
    "SQLite drops the row it cannot read instead of reporting it"
  );
  ...
  let actual = native_data_archive::select_data_archive_series(
    ...
    NativeCancellation::new(),
    DataArchiveColumn::CpuAvg,
    window(&start, &end, 86_400_000, ArchiveBucketTimestamp::Start),
  )
  .await
  .unwrap_err();
  match actual {
    NativeDatabaseError::UnreadableTimestamp { table, timestamp } => {
      assert_eq!(table, "DATA_ARCHIVE");
      assert_eq!(timestamp, "2099-13-45T99:99:99+00:00");
    }
