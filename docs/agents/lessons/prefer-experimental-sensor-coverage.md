...
scope: native sensor enablement, failure diagnostics, specification ownership, and clean-room review
...
root_cause: verification confidence and runtime availability were collapsed into one binary enablement gate, then overcorrected into successful-sample and UI metadata
...
verification: confirm the path is recognized, read-only, uses an existing decode and plausibility gate, keeps verification status in sensor docs, identifies Experimental only on a surfaced failure, and does not guess an address, register map, or chip selection
...
revalidate_when: sensor verification vocabulary, failure presentation policy, clean-room role boundaries, PawnIO module recognition, or native sensor wire models change
---
...
read-only, plausibility-gated decode, enable the path as Experimental. Keep the
Verified/Experimental hardware matrix in the relevant sensor specification.
Successful readings keep the ordinary Core, App, and UI contracts; only a
surfaced failure from the attempted path should identify it as experimental.
