...
| [`cpu-intel-rapl-msr.md`](cpu-intel-rapl-msr.md) | Intel CPU package power via RAPL energy MSRs (`0x606`/`0x611`), unit decode, 32-bit wraparound handling, Silvermont-unit exclusion | Phase 5 | Implementation-ready (rev 1) |
| [`cpu-amd-zen-rapl-msr.md`](cpu-amd-zen-rapl-msr.md) | AMD Zen (17h/19h/1Ah) CPU package power via RAPL energy MSRs (`0xC0010299`/`0xC001029B`), per-model counter widths, width-agnostic modular decode, 1Ah 44h domain-semantics open question | Phase 5 | Implementation-ready (rev 1) |
