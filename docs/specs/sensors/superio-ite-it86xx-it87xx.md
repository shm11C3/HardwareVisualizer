...
| FAN1-3 on exact raw chip ID `0x8728` | Register and formula facts exist, but the active divisor, split-counter consistency, and stopped/invalid values cannot be closed safely from S1 alone. | Disabled. No fan register reads or RPM publication in revision 1. |
| FAN4/5, voltages, PWM/control, limits, alarms/status, GPIO, board-specific physical labels | Outside the revision 1 ready scope. | Disabled. |
...
| Within LDN `0x04`, `CR30` bit 0 is the activation state. Revision 1 reads and requires this bit; it never writes `CR30`. | S1 §8.8.1, printed p. 61 |
...
The revision 1 path is usable only when all of the following hold:
...
- PawnIO `ioctl_find_bars` succeeds after slot selection, ITE
  ...
  base discovery. A failure leaves Environment Controller port access
  unavailable; do not bypass the PawnIO allow-list. (S3)

## Register map (ready temperature scope)

Revision 1 uses no bank-selection write: every enabled register is in the
...
| EC `0x00` | Configuration Register | bits `3`, `0` | Bit 0 `Start`: `1` starts monitoring and `0` selects standby. Bit 3 `INT_Clear`: `1` clears interrupt lines and stops the monitoring loop; the loop resumes after it is cleared. Revision 1 only reads and requires bit 0 = 1 and bit 3 = 0. | Monitoring state | S1 §9.5.2.2.1 and §9.5.3.6, printed pp. 91, 110 |
...
Detection and base authorization are cached on the configured PawnIO
`LpcIO` executor; do not enter configuration mode for every sample.
...
5. While the selected chip and its discovered configuration remain
   ...
   (S3)
6. Run the ITE configuration exit in `finally`, then release the ISA
   mutex. Cache only the exact chip ID, selected slot, validated base,
   and successfully BAR-authorized executor state. Rediscover before
   sampling if that executor/slot state is replaced or invalidated.

Each sample uses the cached, BAR-authorized executor and this order:
...
3. Read EC `0x51`. A physical channel is eligible only when its
   ...
   bits 7:6 (`01` = TMPIN1, `10` = TMPIN2, `11` = TMPIN3); revision 1
...
5. Apply the revision 1 plausibility policy: accept an individual
...
Those facts are **not** a revision 1 implementation surface. S1 does not
...
Consequently revision 1 performs no FAN1-3 register reads and publishes
...
port's writable index field as bits 6:0. Revision 1 does not invent a
...
- The product can route or configure temperature inputs in board-specific
  ways. Revision 1 exposes only generic `TMPIN1`-`TMPIN3` names and does
...
- Non-blocking for Phase 4: No accepted IT8728F hardware dump exists;
  revision 1 intentionally classifies the primary-source-defined,
...
2. Under one bounded ISA-mutex transaction per slot, capture raw ITE
   ...
   `ioctl_find_bars` and PIO HRESULT; and guaranteed ITE exit outcome.
...
Revision 1 satisfies the directory-level ready gate for the scoped
...
Implemented from docs/specs/sensors/superio-ite-it86xx-it87xx.md revision 1 (commit <ready-commit>), limited to the exact 0x8728 / IT8728F/EX Experimental TMPIN1-TMPIN3 scope marked enabled in that revision.
