...
**One deliberate divergence.** A stored timestamp SQLite cannot read as an
instant - a spelling no current writer produces, but one a released build could
have left behind - has no epoch key on either side. SQLite groups such a row
under a NULL bucket, sqlx decodes that NULL as `0`, and bucket 0 is outside
every range a user can ask for: the row is inside the requested range and simply
absent from the answer, with nothing said. The native query refuses instead and
names the stored value. For the screen both outcomes are one row of missing
data; only one of them is findable, and quietly under-reporting a range is the
failure mode this archive work exists to rule out.
