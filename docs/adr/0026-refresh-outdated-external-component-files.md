...
3. **An update of HardwareVisualizer triggers the refresh.** When the MSI
   upgrades an existing installation, a deferred custom action runs the
   refresh from the already elevated install, at every UI level, including the
   in-app updater (`/passive`) and `msiexec /qn` package-manager upgrades. It
   runs only under Program Files, like the setup action. A refresh maintains a
