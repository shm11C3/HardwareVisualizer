...
Storage Health Snapshots are stored and retained separately from the Hardware Archive. Hardware Archive data tracks utilization history for CPU, memory, GPU, and processes, while Storage Health history tracks long-term device health signals that are useful even when utilization history is kept for a shorter period.
