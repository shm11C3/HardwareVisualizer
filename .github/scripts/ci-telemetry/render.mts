...
// Per-thread values alone cannot prove a job was serialized (the OS can
// migrate a single-threaded process across cores over one sampling
// interval, so it can look like an even split across threads), so this is
// the number that actually answers "how parallel was this job": the
// average count of threads doing work, derived from data already in the
// marker rather than a new stored figure. See schema.mts's CpuThreadStat
// doc comment and docs/development/ci-telemetry.md.
function averageBusyThreadsCell(avg: number, cpuCount: number): string {
  const busyThreads = (avg * cpuCount) / 100;
  return `${busyThreads.toFixed(1)}/${cpuCount}T`;
}

// Rounded per-thread averages in index order for <=8 threads (matches the
// timeline's own cap), with an unmeasured thread (null: no usable delta
// pair for that thread specifically) rendered as "–" so its position in the
// list stays visible rather than silently shifting the rest left. A
// min–max range above 8 threads ignores unmeasured entries, so the cell
// cannot grow unbounded with core count. Returns null (no second line) when
// every thread is unmeasured — a row of nothing-but-dashes has nothing to
// show.
function threadLine(
  cpuThreads: JobResourcesMarker["cpu_threads"],
): string | null {
  if (!cpuThreads || cpuThreads.length === 0) return null;
  ...
  if (rounded.length > 8) {
    return `T ${Math.min(...measured)}–${Math.max(...measured)}`;
  }
  return `T ${rounded.map((value) => (value === null ? "–" : String(value))).join(" · ")}`;
}

function cpuCell(resources: JobResourcesMarker | null): string {
  if (!resources?.cpu_pct) return "–";
  const { avg, p95 } = resources.cpu_pct;
  const { cpu_count: cpuCount } = resources;
  // cpu_count comes from an untrusted marker (markers.mts only checks it is
  // finite and >= 0): 0 or a non-integer has no meaningful "busy threads"
  // figure, so the whole `· x.x/NT` segment is omitted rather than
  // rendering a nonsensical "0.0/0T".
  const busyThreadsPart =
    Number.isInteger(cpuCount) && cpuCount > 0
      ? ` · ${averageBusyThreadsCell(avg, cpuCount)}`
      : "";
  const line1 = `${Math.round(avg)}% / ${Math.round(p95)}%${busyThreadsPart}`;
  const line2 = threadLine(resources.cpu_threads);
  return line2 ? `${line1}<br><sub>${line2}</sub>` : line1;
}
...
function renderBody({
  ...
  lines.push("");
  lines.push(
    "<sub>Generated by the CI telemetry workflow from the GitHub API and job log markers. Queue = time from job creation to start. N/MT = average busy threads of M logical CPUs; T = per-thread average %.</sub>",
  );
